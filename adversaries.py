...
#output_perturbation:(optional) if True, the method returns the adversarial perturbation. If false, the method returns the image plus the perturbation.
#device: the device on which to compute the attack
def pgd_attack_p(input_image,label, epsilon,model, loss_function,p,iterations,step_size=None, rand_init=True, clip_min=None,clip_max=None, intermediate_clip=False, targeted=False,output_perturbation=False,device=torch.device('cpu')):
    image=input_image.detach()
    image.requires_grad=True
