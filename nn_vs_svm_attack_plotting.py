...
def white_box_attacks_plots_vs_eps(eps_list,nets,svms,nets_examples,svms_examples,save_fig_folder=None):
    ...
    for i in range(len(eps_list)):
        ...
        nn_errors_eps=[classification_error_under_attack(nets_examples_list[j],nets_list[j],True) for j in range(len(nets_list))]
        ...
        svm_errors_eps=[classification_error_under_attack(svms_examples_list[j],svms_list[j],False) for j in range(len(svms_list))]
    ...
    variables_dict={"epsilons":eps_list, 
                    "net_mean_errors":net_errors, "net_sds":net_sd,
                    "svm_mean_errors":svm_errors, "svms_sds":svm_sd}
...
def grey_box_attacks_plots_vs_eps(eps_list,nets,svms,nets_examples,svms_examples,save_fig_folder=None):
    net_errors=[0]*len(eps_list)
    net_sd=[0]*len(eps_list)
    svm_errors=[0]*len(eps_list)
    svm_sd=[0]*len(eps_list)
    for i in range(len(eps_list)):
        ...
        #errors and standard deviations of a neural net
        nn_errors_eps=[classification_error_under_attack(svms_examples_list[j],nets_list[j],True) for j in range(len(nets_list))]
        nn_errors_eps=np.asarray(nn_errors_eps)
        net_errors[i]=np.mean(nn_errors_eps)
        net_sd[i]=np.std(nn_errors_eps)

        #errors and standard deviations of the svm
        svm_errors_eps=[classification_error_under_attack(nets_examples_list[j],svms_list[j],False) for j in range(len(nets_list))] 
        svm_errors_eps=np.asarray(svm_errors_eps)
        svm_errors[i]=np.mean(svm_errors_eps)
        svm_sd[i]=np.std(svm_errors_eps)
...
def black_box_attacks_plots_vs_eps(eps_list,nets,svms,nets_examples,svms_examples,save_fig_folder=None):
    ...
    for i in range(len(eps_list)):
        ...
        net_to_net_errors[i],net_to_net_sd[i]=average_non_matching_attack_mean_and_stds(nets_examples[i],nets[i],True)
        svm_to_net_errors[i],svm_to_net_sd[i]=average_non_matching_attack_mean_and_stds(svms_examples[i],nets[i],True)
        net_to_svm_errors[i],net_to_svm_sd[i]=average_non_matching_attack_mean_and_stds(nets_examples[i],svms[i],False)
        svm_to_svm_errors[i],svm_to_svm_sd[i]=average_non_matching_attack_mean_and_stds(svms_examples[i],svms[i],False)
    ...
    for test_index in range(M):
        for attack_index in range(M):
            if test_index != attack_index:
                error=classification_error_under_attack(adversarial_examples[attack_index],test_models[test_index],test_model_neural_net)
                mns.append(error)
    mns=np.asarray(mns)
...
#uncomment these lines to test the code on a small portion of MNIST
    ...
    for i in range(len(eps_list)):
        ...
        for j in range(len(nets_eps)):
            net=nets_eps[j]
            prefix="trial"+str(j)
            examples_name=os.path.join(examples_parent_dir,"nets",prefix,prefix+"_epsilon="+str(eps)+"examples")
            examples=gen_adversarial_examples(net,input_nn_attack,eps,mnist_test_sbset, save=examples_name)
            nets_examples_eps.append(examples)
        for j in range(len(svms_eps)):
            svm=svms_eps[j]
            prefix="trial"+str(j)
            examples_name=os.path.join(examples_parent_dir,"svms",prefix,prefix+"_epsilon="+str(eps)+"examples")
            examples=gen_adversarial_examples(svm,input_svm_attack,eps,mnist_test_sbset, save=examples_name)
